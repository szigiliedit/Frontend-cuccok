...
export function PizzaModPage(props) {
    ...
    const[modfeltet,setModfeltet] = useState('');
    ...
    useEffect(() => {
        (async () => {
            ...
            try {
                const res = await fetch(`http://localhost:8080/pizza/${id}`)
                ...
                setModfeltet(pizza.feltet);
                console.log(modfeltet);
    ...
    }, [id,modname,modgluten,modfeltet,modkepurl]);
    ...
    const modFeltet = event => {
        setModfeltet(event.target.value);
    }
